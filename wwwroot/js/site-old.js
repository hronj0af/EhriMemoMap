...
var mapAPI = {
    ...
    initMap: function (jsonMapSettings) {
        ...
        this.incidentIcon = new L.divIcon({ className: 'leaflet-incident-icon' });
        this.addressIcon = new L.divIcon();
        this.interestIcon = new L.divIcon({ className: 'leaflet-interest-icon' });

        this.bluepointIcon = new L.icon({
    ...
    callBlazor_ShowPlaceInfo: function (event) {
    	var point = event.target._latlng != undefined ? event.target._latlng : event.target._latlngs[0][0][0];
        var bbox = mapAPI.convertObjectPositionToBBoxParameter(point);
    ...
    convertObjectPositionToBBoxParameter: function (point) {

        containerPosition = this.map.latLngToContainerPoint(point);

        var sizeOfBox = 5;// mapAPI.isMobileBrowser() ? 20 : 5;

        var minx = containerPosition.x < sizeOfBox / 2 ? 0 : containerPosition.x - sizeOfBox / 2;
        var miny = containerPosition.y < sizeOfBox / 2 ? 0 : containerPosition.y + sizeOfBox / 2;
        var maxx = containerPosition.x < sizeOfBox / 2 ? 0 : containerPosition.x + sizeOfBox / 2;
        var maxy = containerPosition.y < sizeOfBox / 2 ? 0 : containerPosition.y - sizeOfBox / 2;
