(function (global, factory) {
    ...
    function create_if_block_1$1(ctx) {
    	...
    	return {
    		c() {
    			img = element("img");
    			attr(img, "sizes", img_sizes_value = /*opts*/ ctx[8].sizes || `${/*sizes*/ ctx[2]}px`);
    			attr(img, "alt", img_alt_value = /*activeItem*/ ctx[1].alt);
    		},
    		m(target, anchor) {
    		...
    		p(ctx, dirty) {
    			if (!current || dirty[0] & /*sizes*/ 4 && img_sizes_value !== (img_sizes_value = /*opts*/ ctx[8].sizes || `${/*sizes*/ ctx[2]}px`)) {
    				attr(img, "sizes", img_sizes_value);
    			}

    			if (!current || dirty[0] & /*activeItem*/ 2 && img_alt_value !== (img_alt_value = /*activeItem*/ ctx[1].alt)) {
    ...
    function create_if_block$1(ctx) {
    	...
    	return {
    		...
    		p(ctx, dirty) {
    			...
    			if (dirty[0] & /*activeItem*/ 2) loading_changes.activeItem = /*activeItem*/ ctx[1];
    ...
    function create_fragment$3(ctx) {
    	...
    	return {
    		c() {
    			...
    			toggle_class(div0, "bp-drag", /*pointerDown*/ ctx[5]);
    			toggle_class(div0, "bp-canzoom", /*maxZoom*/ ctx[11] > 1 && /*$imageDimensions*/ ctx[0][0] < /*naturalWidth*/ ctx[12]);
    			set_style(div0, "background-image", getThumbBackground(/*activeItem*/ ctx[1]));
    			set_style(div0, "transform", style_transform);
    			attr(div1, "class", "bp-img-wrap");
    			toggle_class(div1, "bp-close", /*closingWhileZoomed*/ ctx[6]);
    		},
    		m(target, anchor) {
    		...
    		p(ctx, dirty) {
    			...
    			if (!current || dirty[0] & /*pointerDown*/ 32) {
    				toggle_class(div0, "bp-drag", /*pointerDown*/ ctx[5]);
    			}
    			...
    			if (!current || dirty[0] & /*closingWhileZoomed*/ 64) {
    				toggle_class(div1, "bp-close", /*closingWhileZoomed*/ ctx[6]);
    			}
    		},
    		i(local) {
    ...
    function instance$3($$self, $$props, $$invalidate) {
    	...
    	function changeZoom(amt = maxZoom, e) {
    		...
    		imageDimensions.set(newDimensions).then(() => {
    			$$invalidate(2, sizes = Math.round(Math.max(sizes, newWidth)));
    		});
    	...
    	const onPointerMove = e => {
    		if (pointerCache.size > 1) {
    			...
    			$$invalidate(5, pointerDown = false);
    		...
    		if (!$zoomed) {
    			// close if swipe up
    			if (y < -90) {
    				$$invalidate(5, pointerDown = !opts.noClose && props.close());
    			}
    	...
    	function onPointerUp(e) {
    		...
    		if (pinchDetails) {
    			...
    			$$invalidate(5, pointerDown = prevDiff = 0);
    	...
    	const onMount = node => {
    		...
    		const loadImageAndSetDimensions = () => {
    			if (activeItem.img && (!activeItem.width || !activeItem.height)) {
    				const img = new Image();
    				img.src = activeItem.img;

    				img.onload = () => {
    					$$invalidate(1, activeItem.width = img.naturalWidth, activeItem);
    					$$invalidate(1, activeItem.height = img.naturalHeight, activeItem);

    					// Recalculate dimensions and update the image
    					$$invalidate(24, calculatedDimensions = props.calculateDimensions(activeItem));

    					imageDimensions.set(calculatedDimensions, { duration: 0 });
    	...
    	$$self.$$.update = () => {
    		...
    		if ($$self.$$.dirty[0] & /*$closing, $zoomed, calculatedDimensions*/ 117440512) {
    			...
    			if ($closing && $zoomed && !opts.intro) {
    				...
    				$$invalidate(6, closingWhileZoomed = true);
    			}
    		}
    	};
    ...
    // (320:4) {#if activeItem.img}
    function create_if_block_3(ctx) {
    	let imageitem;
    	let current;

    	imageitem = new Image_1({
    			props: {
     ...
     */
    	const calculateDimensions = ({ width = 1920, height = 1080 }) => {
    		...
    		if (imageRatio > containerRatio) {
    			newWidth = container.w * scale;
    			newHeight = newWidth / imageRatio;
    		} else {
    			newHeight = container.h * scale - 2;
    	...
    	const containerActions = node => {
    		...
    		$$invalidate(19, container.w = node.offsetWidth, container);
    		$$invalidate(19, container.h = node.offsetHeight, container);
    		let roActive;
